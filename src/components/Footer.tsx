...
const defaultFooter = {
  tagline: 'Capturing light. Telling stories.',
  copyright: 'Tanjim Rahman Photography',
};

export default function Footer() {
  const { data: footer } = useSiteSettings('footer', defaultFooter);
            ...
            <div className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <button key={link} onClick={() => scrollTo(link.toLowerCase())}
                  className="text-left font-body text-[13px] text-text-muted-warm hover:text-primary transition-colors duration-300">{link}</button>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-col gap-3">
              {socials.map((s) => (
                <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer"
                  className="font-body text-[13px] text-text-muted-warm hover:text-primary transition-colors duration-300">{s.name}</a>
              ))}
            </div>
          </div>
        </div>
